...
export default function SecteurModal({ isOpen, onClose, secteur, cdcs }: { isOpen: boolean, onClose: () => void, secteur: any, cdcs: any[] }) {
    ...
    const onSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (secteur) {
            put(route('admin.secteurs.update', secteur.id), { onSuccess: () => onClose() });
        } else {
            post(route('admin.secteurs.store'), { onSuccess: () => { reset(); onClose(); } });
        }
    };
                        ...
                        <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:w-full sm:max-w-xl">
                            <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                <Dialog.Title as="h3" className="text-xl font-bold leading-6 text-gray-900 mb-4 border-b pb-2">
                                    {secteur ? 'Modifier le Secteur' : 'Nouveau Secteur'}
                                </Dialog.Title>
                                <form onSubmit={onSubmit} className="space-y-4">
                                    <div>
                                        <InputLabel htmlFor="nom" value="Nom du Secteur" />
                                    ...
                                    <div>
                                        <InputLabel htmlFor="cdc_id" value="CDC de rattachement" />
                                        <select id="cdc_id" className="mt-1 block w-full border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-md shadow-sm" value={data.cdc_id} onChange={e => setData('cdc_id', e.target.value)} required>
                                            <option value="">Sélectionnez un CDC</option>
                                            {cdcs.map(c => <option key={c.id} value={c.id}>{c.nom}</option>)}
                                        </select>
                                        <InputError message={errors.cdc_id} className="mt-2" />
                                    </div>
